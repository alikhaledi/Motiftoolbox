...
class system(win.window):
	...
	def load_initial_condition(self, x, y, z): # only one phase:  everything's square!
		X = np.zeros((model.N_EQ4), float)
		phi_x, phi_y, phi_z = tl.PI2*(1.-x), tl.PI2*(1.-y), tl.PI2*(1.-z)
		X[::model.N_EQ1] = self.x_orbit([0., phi_x, phi_y, phi_z])
		X[1::model.N_EQ1] = self.y_orbit([0., phi_x, phi_y, phi_z])
		return X
	...
	def load_initial_conditions(self, initial_phase): # only one phase:  everything's square!
		...
		n2 = n**2
		X = np.zeros((n**3, model.N_EQ4), float)
		...
		for i in xrange(n):
			...
			for j in xrange(n):
				...
				for k in xrange(n):
					X[i*n2+j*n+k, model.N_EQ1:] = np.array([V_i, H_i, self.x_orbit(phi[j]), self.y_orbit(phi[j]), self.x_orbit(phi[k]), self.y_orbit(phi[k])])
