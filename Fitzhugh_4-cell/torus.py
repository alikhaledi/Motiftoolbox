...
class torus(win.window):
	...
	def adjust_grid(self, adjustment):
		self.GRID = int(adjustment.value)
	...
	def focus_in(self, event=None):
		...
		descriptor += "'g' : switch processor (%s)" % (PROCESSOR[self.USE_GPU])
		self.echo(descriptor)
	

	def on_key(self, event):
		try: 		self.key_func_dict[event.key](self)
		except: 	self.key_func_dict[event.key] = lambda x: None
	...
	def reset_torus(self, widget=None, event=None):
...
compute_phase_trace = [torus.compute_phase_trace_cpu, torus.compute_phase_trace_gpu]
